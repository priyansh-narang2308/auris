...
import { refreshTokenifNeed } from "@/lib/integrations/refreshTokenIfNeeded";
...
export async function POST(request: NextRequest) {
  ...
  if (platform === "jira" || platform === "asana") {
    try {
      integration = await refreshTokenifNeed(integration);
    } catch (error) {
  ...
  try {
    if (platform === "trello") {
      if (!integration.boardId) {
        return NextResponse.json(
          { error: "Board not configured. Please configure your port." },
          { status: 400 },
        );
      }
