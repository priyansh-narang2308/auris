...
export function useMeetingDetail() {
  ...
  const chat = useChatCore({
    ...
    getRequestBody: (input) => ({
      meetingId,
      question: input, //paass this both as the input
    }),
  });
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    chat.handleInputChange(suggestion);
  };
  ...
  // A useeffect for the processing of transcripts
  useEffect(() => {
      ...
      try {
        ...
        if (
          ...
          if (typeof meeting.transcript === "string") {
            ...
            // take the whole meeting transcript
            transcriptText = meeting.transcript
              .map(
                (segment: any) =>
        ...
        console.error("Error in the RAG processing:", error);
      }
    };
