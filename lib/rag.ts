...
export async function processTranscript(
  ...
  const vectors = chunks.map((chunk, index) => ({
    // the id value and the metadata for pincone right
...
export async function chatWithMeeting(
  ...
  const context = results
    .map((result) => {
      const speaker = result.metadata?.speakerName || "Unknown Speaker";
      const content = result.metadata?.content || "";
      return `${speaker}: ${content}`;
    })
    .join("\n\n");

  const systemPrompt = `You are helping someone understand and analyze their meeting.
    Meeting: ${meeting?.title || "Untitled Meeting"}
    Date: ${meeting?.createdAt ? new Date(meeting.createdAt).toDateString() : "N/A"}

    Here's what was discussed:
    ${context}

    Answer the user's question based only on the meeting content above. If the answer isn't in the meeting, say so that it's not. Be proffessional`;
